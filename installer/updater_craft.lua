...
if not component.isAvailable("internet") then error("Internet card required") end
...
local function getFileList()
  ...
  if fs.exists(tmp) then
    ...
    for line in io.lines(tmp) do
      line = line:gsub("\r","")
      if line ~= "" and not line:match("^#") then table.insert(t, line) end
    end
    fs.remove(tmp)
...
local function update()
  ...
  log("Репозиторий: "..REPOSITORY, COL_DIM)

  local files = getFileList()
  local total = #files
  if total == 0 then
    writeStatus("Нет файлов для обновления.", COL_WARN)
    text(X+2, Y+H-2, "Нажми любую клавишу…", COL_TEXT)
    event.pull("key_down"); return
  end
  ...
  for i, rel in ipairs(files) do
    ...
    if ok then okCount=okCount+1; log("OK: "..rel, COL_OK)
    else failCount=failCount+1;   log("ERROR: "..rel, COL_ERR) end
  ...
  if failCount == 0 then
    writeStatus("Обновление завершено успешно.", COL_OK)
  else
    writeStatus("Завершено с ошибками. См. лог.", COL_WARN)
  end
