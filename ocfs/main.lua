-- main.lua (рабочий старт + понятные ошибки)
local shell = require("shell")
package.path = package.path .. ";" .. shell.getWorkingDirectory() .. "/?.lua;" .. shell.getWorkingDirectory() .. "/?/init.lua"

local function req(name)
  local ok, mod = pcall(require, name)
  if not ok then io.stderr:write("[require fail] "..name..": "..tostring(mod).."\n"); os.exit(1) end
  return mod
end
...
local function app()
  term.clear()
  if gpu then pcall(gpu.setResolution, 120, 40) end
...
local ok, err = xpcall(app, debug and debug.traceback or function(e) return e end)
if not ok then io.stderr:write("[runtime error] "..tostring(err).."\n") end
